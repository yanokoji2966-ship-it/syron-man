-- SYRON Man - AUTOMAÇÃO DE RESTAURAÇÃO DE ESTOQUE
-- Este script garante que o estoque volte ao normal se um pedido for cancelado ou deletado.

-- 1. FUNÇÃO PARA RESTAURAR ESTOQUE AO CANCELAR PEDIDO
...
-- 2. FUNÇÃO PARA RESTAURAR ESTOQUE AO DELETAR UM ITEM OU PEDIDO
CREATE OR REPLACE FUNCTION restore_stock_on_item_delete()
RETURNS TRIGGER AS $$
BEGIN
    -- Se o item for deletado (ou o pedido inteiro for excluído), devolve para o estoque
